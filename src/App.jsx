...
function processStrategy(num, strategy, baseBet) {
  const s = { ...strategy, covered: [...strategy.covered], balances: [...strategy.balances] };
  ...
  s.balances.push(Number(s.balance.toFixed(4)));
  return s;
}
...
export default function App() {
  ...
  const simulateEvent = () => {
    ...
    const updatedSession = [...sessionItems, randomValue];
    setSessionItems(updatedSession);
    setHistory((prev) => [...prev.slice(-11), Math.floor(60 + Math.random() * 35)]);
            ...
            <div className="grid lg:grid-cols-3 gap-6 mt-8">
              <div className="lg:col-span-2 bg-[#08152b] border border-slate-800 rounded-3xl p-6">
                ...
                <div className="flex flex-wrap gap-3 mt-8">
                  <input type="number" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Enter event value 0-36" className="bg-[#020817] border border-slate-700 rounded-2xl px-5 py-4 flex-1 min-w-[220px] outline-none" />
                  <button onClick={addEvent} className="bg-emerald-500 hover:bg-emerald-400 transition px-6 py-4 rounded-2xl font-semibold text-black">Add Event</button>
                  <button onClick={() => setSessionItems((prev) => prev.slice(0, -1))} className="bg-slate-700 hover:bg-slate-600 transition px-6 py-4 rounded-2xl">Undo</button>
                  <button onClick={() => { setSessionItems([]); setHistory(DEFAULT_HISTORY); }} className="bg-rose-900/50 border border-rose-500/30 hover:bg-rose-800/50 transition px-6 py-4 rounded-2xl">Reset</button>
                </div>
              </div>
  ...
  return (
    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
      <div>
        ...
        <p className="text-slate-400 mt-4 max-w-2xl">Manual input tracking, live signal analysis, strategy modules, CSV export and adaptive decision-support logic.</p>
      </div>
...
function RouletteModule({ strategyA, strategyB, rouletteInput, setRouletteInput, processRouletteSpin, resetRoulette, rouletteChartData, spinCount }) {
  return (
  ...
  return (
    <div className="bg-[#08152b] border border-slate-800 rounded-3xl p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
        ...
        <StatBox label="Losses" value={strategy.consecutiveLosses} />
        <StatBox label="Level" value={strategy.martingaleLevel} />
      </div>
...
function TabButton({ active, children, onClick }) {
  return <button onClick={onClick} className={`px-5 py-3 rounded-2xl font-bold border transition ${active ? "bg-cyan-500 text-black border-cyan-400" : "bg-[#08152b] border-slate-800 text-slate-300 hover:bg-slate-800"}`}>{children}</button>;
}
...
function StatBox({ label, value }) {
  return (
    <div className="bg-[#0d1524] border border-slate-800 rounded-2xl p-4">
      ...
      <div className="text-3xl md:text-4xl font-black mt-2">{value}</div>
    </div>
  );
}
